...
def add_team(teams_list):
    raw_input = input("Nhập danh sách các đội bóng/đội tuyển (cách nhau bởi dấu phẩy): ").strip()
    if raw_input == "":
...
def add_schedule(teams_list):
    ...
    for index, match_name in enumerate(matches_list, start=1):
        print(f"{index}. {match_name}")
        
    print(f"Tổng số trận đấu: {len(matches_list)} trận.")
    return matches_list
